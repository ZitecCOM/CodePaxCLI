...
abstract class CodePax_DbVersioning_SqlEngines_Abstract
{
    ...
    {
        exec($this->postFormatString($_shell_command), $output, $exit_code);
        $command_result = implode(" ", $output);
        ...
        if (($this->isError($command_result) === true) || $exit_code != 0) {
    ...
    public function generateTestData($_target_file)
    {
        if (defined('USE_TEST_DATA_COMPRESSION') && USE_TEST_DATA_COMPRESSION === true) {
            $target_sql_file = str_replace(CodePax_DbVersioning_Files_Manager::ARCHIVE_FILE_EXTENSION,
                CodePax_DbVersioning_Files_Manager::SQL_FILE_EXTENSION, $_target_file);
    ...
    {
        if (defined('USE_TEST_DATA_COMPRESSION') && USE_TEST_DATA_COMPRESSION === true) {
            $sql_file = str_replace(CodePax_DbVersioning_Files_Manager::ARCHIVE_FILE_EXTENSION,
                CodePax_DbVersioning_Files_Manager::SQL_FILE_EXTENSION, $_source_file);
